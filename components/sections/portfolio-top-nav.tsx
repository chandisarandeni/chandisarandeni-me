          ...
          <ul className="nav-scroll order-2 flex snap-x snap-mandatory items-center gap-1.5 overflow-x-auto pb-1 md:order-1 md:pb-0">
            {navLinks.map((link) => (
              <li key={link.id} className="shrink-0 snap-start">
                <a
                  href={`#${link.id}`}
                  className="nav-chip nav-underline tap-target inline-flex items-center rounded-full border border-transparent px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-fg hover:border-border-muted hover:bg-surface-strong hover:text-app-fg sm:text-xs"
                >
                  {link.label}
                </a>
              </li>
